...
public final class RemoveCartLine {
    ...
    public RemoveCartLine(UnitOfWork unitOfWork, CurrentCart currentCart,
    ...
    public Result<Cart> execute(Visitor visitor, String lineId) {
        return unitOfWork.inTransaction(() -> {
            Cart cart = currentCart.forVisitor(visitor);
            return cart.removeLine(lineId, clock.now()).map(cartRepository::save);
        });
    }
}
