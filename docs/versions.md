...
|---|---|---|---|
| .NET | 10, patch 10.0.11, LTS to 14 November 2028. The C# backend was built with SDK 10.0.303 on runtime 10.0.11 | 6 September 2026, the SDK on 9 September 2026 | dotnet.microsoft.com support policy, `dotnet --info` |
...
| Kotlin backend | Gradle 9.5.0 through the wrapper with the distribution checksum verified, the highest Gradle the Kotlin 2.4 plugin documents, Kotlin 2.4.10 with the `jvm`, `plugin.spring` and `plugin.jpa` plugins, Spring Boot 4.1.1 with `io.spring.dependency-management` 1.1.7, Kotlin and kotlinx.coroutines 1.11.0 raised above the bill of materials through `bomProperty`, `tools.jackson.module:jackson-module-kotlin` on Jackson 3.1.5, the rest of the Spring managed set as in the Java row, `bcprov-jdk18on` 1.85.2 pinned by hand for `Argon2PasswordEncoder` with the same Argon2id parameters, H2 2.4.240 in PostgreSQL mode as the default database, Testcontainers left out because it cannot run without Docker, `actions/setup-java@v6` with Temurin 25 in the workflow | verified 9 September 2026 | the kotlinlang.org compatibility table, Maven Central, the Spring Boot bill of materials |
| Node federated backend | `@apollo/server` 5.5.1, `@apollo/subgraph` 2.15.0, `@apollo/gateway` 2.14.4 with `@apollo/composition` and `@apollo/federation-internals` 2.14.4, `@as-integrations/express5` 1.1.2 on `express` 5.2.1 and `cors` 2.8.6, `graphql` 16.14.2, `dataloader` 2.2.3, `jose` 6.2.12, `argon2` 0.45.1 (its prebuilt binary resolves with npm install scripts blocked), `pg` 8.23.0 for the unverified PostgreSQL adapter, `node:sqlite` for the default, `@graphql-codegen/cli` 7.4.0 with the `typescript` and `typescript-resolvers` plugins 6.1.0 and `add` 7.1.0, TypeScript 6.0.3, `@types/node` 26.5.0 (the React Router front pins the 24 line to match the runtime, this backend follows at the next dependency pass), Apollo Router 2.16.3 as the version `router/router.yaml` is written for and not run here, Node 24.20.0 | verified 9 September 2026 | npm, GitHub releases for the router |
